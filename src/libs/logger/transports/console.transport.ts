...
export default class ConsoleTransport {
    ...
    return new winston.transports.Console({
      format: winston.format.combine(
        winston.format.printf((log) => {
          const color = this.mapLogLevelEnumColor(log.level as LogLevelEnum);
  ...
  private static mapLogLevelEnumColor(level: LogLevelEnum): LogColors {
    switch (level) {
